...
| **B** | **`$` 접두 jQuery/DOM 객체**를 append하는 삽입 sink (`$el.append($img)`) | 만들어진 노드 append는 HTML 주입 아님 | 스니펫의 **모든** 삽입이 객체 append일 때만 강등 — 템플릿/문자열/`$(…)`/HTML append가 하나라도 있으면 유지 |
| **C** | base64 키 본문 없는 `-----BEGIN … PRIVATE KEY-----` **마커만** | PEM 파싱/라벨 코드, 유출된 키 아님 | 실제 키(스니펫에 base64 본문)는 절대 강등 안 함 |
...
- **HTML 리포트** — 노이즈는 **기본적으로 숨김**(배너에 "실제 N개 표시 · 벤더/오탐 M개 숨김" 표기), 토글로 펼침(리포트엔 그대로 유지). 각 sink는 **DANGEROUS VALUE** 라인으로 sink에 도달하는 정확한 값을 표기하고, **코드 스니펫이 그 값 기준으로 앵커**되어(여러 줄 HTML 템플릿 깊숙이 있어도) 하이라이트로 보이게 함.

> 예: 실제 145개 발견 스캔에서 102개가 벤더/오탐 노이즈로 강등됐지만 확정 XSS 흐름과 위험 필드 주입은 **전부** 유지되고 최상단에 랭크됨.
