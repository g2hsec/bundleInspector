...
각 자산은 정규화(beautify, 소스맵 해석)되고 AST로 파싱된 뒤, **엔드포인트·시크릿·내부 도메인·피처 플래그·디버그 엔드포인트**를 추출하는 룰 엔진을 거치며, 6가지 공격 정찰 고도화로 보강되고 `P0–P3` 위험 등급이 매겨집니다.
...
- **종료 코드.** CLI는 **발견이 있어도 성공 시 `0`**, 오류/중단 시에만 `1`을 반환합니다 — 심각도 기반 종료 게이트는 없습니다. CI에서는 종료 코드가 아니라 리포트/SARIF로 통과 여부를 판정하세요.
...
| `--cookies-from`이 쿠키를 못 가져옴 | DB(및 `-wal` 사이드카)를 먼저 복사하므로 브라우저를 켜둔 채로 가능. 그래도 비어 있으면 값이 OS 암호화된 것 — 쿠키 확장으로 JSON 내보내 `--cookies-file` 사용 |
