...
Node/acorn이 없거나 실패하면 BundleInspector는 조용히 `esprima`로 폴백합니다 — 네이티브 경로가 탐지율을 떨어뜨릴 일은 없습니다. esprima 경로는 파싱 전에 **ES2020 문법(옵셔널 체이닝 `?.`, nullish `??`/`??=`)을 정규화**하므로 최신 단일 라인 난독화 번들도 완전한 AST를 만듭니다 — 이 정규화가 없으면 파싱 불가능한 `?.` 하나가 파일 전체를 문자열-only 폴백으로 무너뜨려 AST 기반 탐지(엔드포인트·sink·taint·flag·debug)를 전부 0으로 만듭니다.
