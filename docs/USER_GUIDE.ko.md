...
- **HTML 리포트** — 노이즈는 흐리게 + `LIKELY FP`/`3p:` 배지로 하위 정렬, **“🚫 Hide vendor / likely-FP noise”** 토글로 숨김, 실제 발견이 먼저 렌더. 각 sink는 **DANGEROUS VALUE** 라인으로 sink에 도달하는 정확한 값(확인할 서버/사용자 필드)을 표기.
