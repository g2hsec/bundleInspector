...
If Node/acorn is missing or anything fails, BundleInspector silently falls back to `esprima` — the native path can never reduce detection. The esprima path **normalizes ES2020 syntax** (optional chaining `?.`, nullish `??`/`??=`) before parsing, so a modern minified single-line bundle still yields a full AST — without it, one unparseable `?.` collapses the file to a string-only fallback and zeroes every AST-based detector (endpoints, sinks, taint, flags, debug).
