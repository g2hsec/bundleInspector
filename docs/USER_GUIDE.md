...
|---|---|---|
| **Endpoint** | INFO | `fetch`/`axios`/`request`/`ajax` calls, `obj.get/post/...`, `XMLHttpRequest.open`, `axios.create` baseURL/default headers, and URL literals matching `/api/`, `/v\d+/`, `/graphql`, `/rest/`, `/rpc/`, `/ws/`, `/socket`, `/webhook`. Statically resolves URLs across template literals, concatenation, ternaries, constants, named objects, and `new URL()`/`new Request()`. |
