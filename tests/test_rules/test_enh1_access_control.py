...
def test_positive_if_role_gated_high():
    f = _one('function h(user){ if(user.isAdmin){ fetch("/api/v1/admin/users"); } }',
             "/api/v1/admin/users")
    assert _is_gated(f)
...
def test_early_return_negative_guard_single_line():
    ...
    f = _one('function h(u){ if(!u.hasRole("admin")) return; axios.delete("/api/v1/users/42"); }',
             "/api/v1/users/42")
    assert _is_gated(f)
...
def test_ternary_flag_gated_medium():
    f = _one('function h(flags){ const r = flags.canDeleteUsers ? fetch("/api/v1/delete") : null; return r; }',
             "/api/v1/delete")
    assert _is_gated(f)
    assert f.metadata["guard_source"] == "ternary"
    # a flag guard is not role/permission/entitlement -> stays at the configured target (medium)
    assert f.metadata["guard_kind"] in ("flag", "generic-authz")
    assert f.severity == Severity.MEDIUM


def test_logical_and_permission_gated():
    f = _one('function h(user){ user.permissions.canView && fetch("/api/v1/reports"); }',
             "/api/v1/reports")
    assert _is_gated(f)
...
def test_entitlement_gated_high():
    f = _one('function h(ctx){ if(ctx.entitlements.exportData){ fetch("/api/v1/export"); } }',
             "/api/v1/export")
    assert _is_gated(f)
...
def test_websocket_gated():
    f = _one('function h(u){ if(u.role==="admin"){ new WebSocket("wss://rt.acme.io/admin"); } }',
...
def test_nested_guards_pick_innermost_authz():
    ...
    f = _one('function h(u,ready){ if(u.isAdmin){ if(ready){ fetch("/api/v1/x"); } } }',
             "/api/v1/x")
    assert _is_gated(f)
    assert f.metadata["guard_kind"] == "role"
...
def test_negative_non_authz_conditions_stay_ungated():
    fs = _endpoints('function h(res,i,n){ if(res.ok){ fetch("/api/v1/public"); } '
...
def test_negative_loading_state_stays_ungated():
    f = _one('function h(state){ if(state.loading){ fetch("/api/v1/spinner"); } }', "/api/v1/spinner")
...
def test_negative_else_branch_not_gated():
    ...
    f = _one('function h(u){ if(u.isAdmin){ adminApi(); } else { fetch("/api/v1/public"); } }',
             "/api/v1/public")
    assert not _is_gated(f)
...
@pytest.mark.parametrize("tokens,expected", [
    ...
    (["featureBetaConsole", "feature"], "feature"),
    (["flags", "toggle"], "flag"),
...
def test_config_disable_flag_suppresses_gating():
    ...
    f = _one('function h(user){ if(user.isAdmin){ fetch("/api/v1/admin/users"); } }',
             "/api/v1/admin/users", config=cfg)
    assert not _is_gated(f)
    assert f.severity == Severity.INFO  # unchanged when disabled


def test_config_target_severity_respected_for_flag():
    cfg = Config()
    cfg.rules.client_side_gating_severity = "low"
    f = _one('function h(flags){ if(flags.betaToggle){ fetch("/api/v1/beta"); } }',
             "/api/v1/beta", config=cfg)
    assert _is_gated(f)
    # flag kind uses the configured target, which we lowered to "low"
