...
def test_graphql_gql_tag_named_operation():
    f = _find(
        "const Q = gql`query GetUser($id: ID!) { user(id: $id) { id name } }`;", "graphql_operation"
    )[0]
...
def test_graphql_mutation():
    f = _find(
        "const M = gql`mutation DeleteAccount { deleteAccount { ok } }`;", "graphql_operation"
    )[0]
    assert (
        f.metadata["operation_type"] == "mutation" and f.extracted_value == "mutation DeleteAccount"
    )
...
def test_graphql_fetch_body_coexists_with_endpoint():
    out = _find(
        "fetch('/graphql',{method:'POST',body:JSON.stringify({query:'query Ping { ping }'})});"
    )
    assert any(
        f.value_type == "api_endpoint" and f.extracted_value == "/graphql" for f in out
    )  # not suppressed
    assert any(
        f.value_type == "graphql_operation" and f.extracted_value == "query Ping" for f in out
    )


def test_graphql_alias_and_fragment_ignored():
    ops = _find(
        "const Q = gql`query { a: user { id } admin { id } } fragment F on User { id }`;",
        "graphql_operation",
    )
    assert len(ops) == 1
    assert ops[0].metadata["fields"] == [
        "user",
        "admin",
    ]  # alias 'a' -> 'user'; fragment not emitted
...
def test_graphql_rejects_json_and_urls():
    assert (
        _find(
            "const j = JSON.stringify({query:{a:1}}); const u = {query:'/api/search?q=1'};",
            "graphql_operation",
        )
        == []
    )
...
def test_ws_send_json_stringify_type():
    f = _find(
        "const ws=new WebSocket('wss://h/s'); ws.send(JSON.stringify({type:'subscribe'}));",
        "ws_message",
    )[0]
...
def test_ws_socketio_emit_event():
    msgs = _find(
        "const socket=io('https://h'); socket.emit('chat:message',p); socket.emit('admin:impersonate',id);",
        "ws_message",
    )
...
def test_ws_enum_type_resolved():
    f = _find(
        "const MT={PING:'ping'}; const ws=new WebSocket('wss://h/s'); ws.send(JSON.stringify({type:MT.PING}));",
        "ws_message",
    )[0]
...
def test_ws_receiver_fp_guard():
    assert (
        _find("res.send('ok'); response.send(html); bus.emit('internal-metric', 1);", "ws_message")
        == []
    )
