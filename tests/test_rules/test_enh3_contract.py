...
def _contracts(source: str):
    ...
    return [f for f in engine.analyze(ir, ctx)
            if f.value_type == "api_endpoint" and "request_contract" in f.metadata]


def test_contract_fetch_bearer_json():
    c = _contracts('const t=getToken(); fetch("https://api.acme.io/v1/users",{method:"POST",'
                   'headers:{"Authorization":`Bearer ${t}`,"Content-Type":"application/json"},'
                   'body:JSON.stringify({name:"x",age:30,active:true})});')[0].metadata["request_contract"]
...
def test_contract_axios_create_default_header_merge():
    c = _contracts('const api=axios.create({baseURL:"https://api.acme.io",headers:{"x-api-key":"KEY"}}); '
                   'api.get("/orders",{params:{page:1,q:"a"}});')[0].metadata["request_contract"]
...
def test_contract_axios_post_arg_positions():
    c = _contracts('axios.post("https://api.acme.io/v2/items",{sku:"S1",qty:2},'
                   '{headers:{"X-Auth-Token":"T"},params:{dryRun:true}});')[0].metadata["request_contract"]
...
def test_contract_urlencoded_body():
    c = _contracts('fetch("/api/login",{method:"POST",'
                   'headers:{"Content-Type":"application/x-www-form-urlencoded"},'
                   'body:"user=admin&pass=secret"});')[0].metadata["request_contract"]
    assert c["body"]["kind"] == "urlencoded"
    assert set(c["body"]["shape"]) == {"user", "pass"}


def test_contract_secret_redaction_default():
    c = _contracts('fetch("https://api.acme.io/admin",{headers:{"Authorization":"Bearer AKIAIOSFODNN7EXAMPLE"}});')[0].metadata["request_contract"]
...
def test_no_contract_on_non_call_endpoints():
    ...
    ctx = AnalysisContext(file_url="f", file_hash="h", source_content='const p="/api/v1/orders"; new WebSocket("wss://h/socket");')
    engine = RuleEngine(Config().rules); engine.register_defaults()
...
def test_poc_build_curl_and_fetch():
    c = _contracts('fetch("https://api.acme.io/v1/users",{method:"POST",'
                   'headers:{"Content-Type":"application/json"},body:JSON.stringify({name:"x"})});')[0].metadata["request_contract"]
    curl = build_curl(c)
