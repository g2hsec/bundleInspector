...
def test_redundant_api_path_removed_but_endpoint_and_sibling_kept():
    src = ('const base="https://api.acme.com";const c=axios.create({baseURL:base});'
...
def test_lone_api_path_kept_when_no_endpoint():
    ...
    assert "/api/v1/users" in {f.extracted_value for f in _by(findings, Category.ENDPOINT, "api_path")}
