...
def _f(cat, vt, line, sev=Severity.HIGH, meta=None, val="", file="app.js"):
    return Finding(
        ...
        title=vt,
        evidence=Evidence(file_url=f"https://x/{file}", file_hash="h", line=line),
        value_type=vt,
        extracted_value=val,
        metadata=meta or {},
    )
...
def _report():
    tf = _f(
        ...
        1077,
        meta={
            "source_kind": "ajax_response",
            "source_line": 1076,
            "sink": ".append()",
            "sink_line": 1077,
            "sink_attr": "",
            "sink_source": "restock.image_url",
            "flow_path": [
                "source: server response @L1076",
                "tainted value `restock.image_url`",
                "sink: .append() @L1077",
            ],
        },
    )
    ...
    up = _f(
        Category.UPLOAD, "client_side_file_validation", 166, sev=Severity.MEDIUM, val="allowedExt"
    )
    ...
    cor = Correlation(
        ...
        source_finding_id=up.id,
        target_finding_id=sink2.id,
...
def test_confirmed_supersedes_candidate_at_same_sink():
    ...
    rpt.correlations.append(
        Correlation(
            edge_type=EdgeType.TAINT,
            source_finding_id=up.id,
            target_finding_id=ind.id,
            confidence=Confidence.MEDIUM,
            reasoning="upload -> sink",
        )
    )
...
def test_render_labels_code_injection_for_an_eval_sink():
    ...
    tf = _f(
        ...
        meta={
            ...
            "sink": "eval()",
            "sink_line": 50,
            "sink_source": "location.hash",
            "flow_path": ["source @L49", "location.hash", "eval() @L50"],
        },
    )
...
def test_render_labels_open_redirect_for_a_navigation_sink():
    ...
    tf = _f(
        ...
        meta={
            ...
            "sink": "location.href=",
            "sink_line": 50,
            "sink_source": "location.search",
            "flow_path": ["source @L49", "location.search", "location.href= @L50"],
        },
    )
    txt = render_chains(build_chains(Report(findings=[tf], correlations=[])))
...
def test_no_crash_on_empty_and_malformed():
    ...
    bad = Report(
        findings=[_f(Category.SINK, "taint_flow", 5)],
        correlations=[
            Correlation(
                edge_type=EdgeType.TAINT,
                source_finding_id="nope",
                target_finding_id="nope2",
